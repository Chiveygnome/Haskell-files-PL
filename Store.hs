...
merge :: Store a b -> Store a b -> Store a b
merge (Store s)  (Store s1) = Store(\key ->if( (isNothing( s key)) && (isNothing(s1 key )))
                                  then do Nothing 
                             else if( not(isNothing ( s key)) && isNothing ( s1 key))
                                  then do  s key 
                             else if(isNothing (s key) && not(isNothing (s1 key)))
                                  then do s1 key
                             else do  s key)